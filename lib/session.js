...
Session.prototype.copy = function(src, dest, vars, options, callback) {
  ...
  function buildScpCmdLine (opts, pemFile) {
    var pemArg = ' -i ' + pemFile
    if (self._options.rsync)
      return util.format('env RSYNC_CONNECT_PROG=\"ssh%s%s\" rsync -avzP', opts, pemFile?pemArg:'')
    return util.format('scp%s%s', opts, pemFile?pemArg:'')
  }
  ...
  if(this._auth.pem) {
    ...
    command = util.format('sshpass -p %s %s %s %s@%s:%s',
              this._auth.password, buildScpCmdLine(self._sshOptions), copyFile, this._auth.username, this._host, dest);
    startProcessing();
  } else {
  ...
  function afterPemFileWritten(err) {
    if(err) {
      ...
      command = util.format('%s %s %s@%s:%s',
                buildScpCmdLine(self._sshOptions, pemFile), copyFile, self._auth.username, self._host, dest);
      startProcessing();}
  }
