...
Session.prototype.copy = function(src, dest, vars, callback) {
  ...
  if(this._auth.pem) {
    ...
    command = util.format('sshpass -p %s scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no %s %s@%s:%s', 
  ...
  function afterPemFileWritten(err) {
    if(err) {
      ...
      command = util.format('scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i %s %s %s@%s:%s', 
...
Session.prototype.execute = function(shellCommand, callback) {
  ...
  if(this._auth.pem) {
    ...
    command = util.format('sshpass -p %s ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no %s@%s "bash -s" < %s', 
  ...
  function afterPemFileWritten(err) {
    if(err) {
      ...
      command = util.format('ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i %s %s@%s "bash -s" < %s', 
